...
async function handleSpotify(searchParams) {
  ...
  let apiRes;
  try {
    apiRes = await fetch(`https://api.ikyyxd.my.id/download/spotifydl?url=${encodeURIComponent(spotifyUrl)}`);
  } catch {
    return jsonError('Gagal menghubungi layanan Spotify.', 502);
  }

  if (!apiRes.ok) {
    // Server sumbernya sendiri lagi down/error (misal 502/503 dari Cloudflare),
    // bukan masalah link Spotify yang dikirim user.
    return jsonError(`Layanan Spotify sedang bermasalah (kode ${apiRes.status}). Coba lagi beberapa saat lagi.`, 502);
  }
  ...
  try {
    ...
    return jsonError('Layanan Spotify sedang bermasalah. Coba lagi beberapa saat lagi.', 502);
  }

  const d = apiData.result || apiData.data || apiData;
  const dlUrl = d && (d.download || d.url || d.downloadUrl || d.link);
  if (!dlUrl) {
    const apiMsg = apiData.message || apiData.msg || apiData.error;
    return jsonError(
      apiMsg ? `Gagal memproses link: ${apiMsg}` : 'Gagal memproses link. Layanan sumber tidak menemukan file untuk link ini.',
      502
    );
  }
