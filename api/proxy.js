...
async function handleSpotify(searchParams) {
  ...
  // Kalau tahap ambil file (streamUpstream) gagal, link download yang
  // dipakai kemungkinan udah basi/CDN-nya lagi bermasalah sesaat. Daripada
  // langsung nyerah, kita ulang dari awal (resolve ulang buat dapet link
  // fresh) sampai maxStreamAttempts kali sebelum bener-bener nyerah.
  ...
  for (let streamAttempt = 1; streamAttempt <= maxStreamAttempts; streamAttempt++) {
    ...
    const res = await streamUpstream(dlUrl, filename, {
...
async function handleSpotifySearch(searchParams) {
  ...
  // Sama seperti handleSpotify: kalau streamUpstream gagal, ulang dari
  // awal (resolve ulang buat dapet link fresh) sampai maxStreamAttempts
  // kali sebelum bener-bener nyerah.
  const maxStreamAttempts = 2;
  let lastResponse = null;
...
async function handleHealth() {
  ...
  try {
    ...
    // Server merespons apa pun (termasuk 404/403) berarti servernya hidup.
    // Yang dianggap "offline" cuma kalau fetch-nya gagal total (network
    // error/timeout) di blok catch di bawah, bukan berdasar status code.
    spotify = !!res;
  } catch {
    spotify = false;
  }
